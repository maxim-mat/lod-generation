...
def test_network_forward_shapes(equivariance):
    ...
    X = torch.nn.functional.one_hot(torch.randint(0, NUM_NODE_CLASSES, (B, N)), NUM_NODE_CLASSES).float()
    R = torch.randn(B, N, 3)
    E = torch.nn.functional.one_hot(torch.randint(0, NUM_EDGE_CLASSES, (B, N, N)), NUM_EDGE_CLASSES).float()
    ...
    assert X_pred.shape == (B, N, NUM_NODE_CLASSES)
